 /*
* Chozibot es una creación de Deyvin.C
* Chozibot no tiene ningun fin de lucro
* Chozibot se reserva todos los derechos de autor
* © 2021 Chozi, INC.
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				only: {
					...
					ownerB: '[❗] Este comando solo lo usa ChoziBot!',
       ...
       /******ENTRADA FIN DE FUNCIONES******/
			function addMetadata(packname, author) {	
				if (!packname) packname = 'ChoziBot'; if (!author) author = 'Shanduy';	
	...
	case 'hidetag':
			...
			case 'tagall':
				...
				client.updatePresence(from, Presence.composing) 
					...
					client.sendMessage(from, '╔══✪〘 Mencionar A Todos 〙✪══\n╠➥'+teks+'╚═〘 ChoziBot 〙', text, {detectLinks: false, quoted: mek})
					break
                        case 'tagall4':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           client.sendMessage(from, options, text, { quoted: mek } )
	        ...
	        if (!isUser) return reply(mess.only.daftarB)
                ...
                play = body.slice(5)
                anu = await fetchJson(`https://api.zeks.xyz/api/ytplaymp3?q=${play}&apikey=hshanduyr`)
               if (anu.error) return reply(anu.error)
